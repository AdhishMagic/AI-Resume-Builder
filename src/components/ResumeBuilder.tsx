...
export const ResumeBuilder: React.FC<ResumeBuilderProps> = ({ onGenerate }) => {
    ...
    const isJdActive = showJdInput || Boolean(jd.trim());
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[420px_1fr] gap-6 items-start">
                ...
                <div className="w-full lg:sticky lg:top-24">
                    <div className="relative group">
                        <div className="absolute -inset-1 bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl blur opacity-0 group-hover:opacity-40 transition duration-1000 group-hover:duration-200"></div>
                        <div className="relative bg-gray-900/50 border border-gray-800 rounded-xl p-4">
                ...
                <div className="relative group">
                    <div
                        className={`absolute -inset-1 bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl blur transition duration-1000 group-hover:duration-200 ${isDragging ? 'opacity-50' : 'opacity-0 group-hover:opacity-40'}`}
                    ></div>
                    <div
                        ...
                        className={`
                            relative border border-dashed rounded-xl p-4 transition-all duration-300 cursor-pointer
                ...
                {/* 2. TEXT + JD SPLIT AREA */}
                <div className={`flex flex-col ${isJdActive ? 'md:flex-row' : ''} gap-3`}>
                    {/* Main input */}
                    <div className={`relative group w-full ${isJdActive ? 'md:w-1/2' : 'md:w-full'}`}>
                        <div className="absolute -inset-1 bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl blur opacity-0 group-hover:opacity-50 transition duration-1000 group-hover:duration-200"></div>
                        <div className="relative bg-gray-900 rounded-xl p-0.5 flex flex-col h-[168px]">
                            <textarea
                                value={description}
                                onChange={(e) => setDescription(e.target.value)}
                                placeholder={file ? "Add any extra details..." : "I'm a Full Stack Developer with 2 years of experience..."}
                                className="w-full flex-1 bg-gray-900 text-sm text-white placeholder-gray-600 p-4 rounded-t-xl border-none focus:ring-0 resize-none leading-relaxed"
                                disabled={isGenerating}
                            />
                            ...
                            <div className="flex justify-between items-center px-4 py-2 border-t border-gray-800 bg-gray-900/50 rounded-b-xl">
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-1.5 text-xs font-medium transition-colors ${isJdActive ? 'text-blue-400' : 'text-gray-500 hover:text-gray-300'}`}
                                >
                                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                                    </svg>
                                    {jd.trim() ? 'JD Added' : 'Add JD'}
                                </button>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button
                                        ...
                                        className={`
                                                flex items-center justify-center px-3 py-2 rounded-lg text-sm font-semibold text-white transition-all duration-300
                                            ${isGenerating
                    ...
                    {/* JD input (hidden until Add JD; stays visible if JD has content) */}
                    {isJdActive && (
                        <div className="w-full md:w-1/2">
                            <div className="relative group h-full">
                                <div className="absolute -inset-1 bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl blur opacity-0 group-hover:opacity-50 transition duration-1000 group-hover:duration-200"></div>
                                <div className="relative bg-gray-900 rounded-xl p-0.5 h-[168px]">
                                    <div className="absolute top-2 right-2 z-10 transition-opacity duration-200">
                                        <VoiceInput
                                            onTranscript={(text) => setJd(prev => (prev ? prev + ' ' : '') + text)}
                                        />
                                    </div>
                                    <textarea
                                        value={jd}
                                        ...
                                        className="w-full h-full bg-gray-900 text-sm text-white placeholder-gray-600 p-4 rounded-xl border-none focus:ring-0 resize-none leading-relaxed"
                                        disabled={isGenerating}
                                    />
                                </div>
                            </div>
